...
# Pritify Help argument output with bat
...
# Run Firestorm For Secondife
alias 'slife'="prime-run firestorm"

# Bind to interface to bypass wireguard
alias 'b2i'="BIND_INTERFACE=wlan0 DNS_OVERRIDE_IP=8.8.8.8 LD_PRELOAD=/usr/lib/bindToInterface.so"

# Alias to connect adb to a specific phone IP and dynamically discovered open port
alias connect_phone='env PHONE=192.168.0.102 bash -c '\''HOME="$XDG_DATA_HOME"/android adb connect ${PHONE}:$(nmap -sT ${PHONE} -p30000-49999 | awk -F/ "/tcp open/{print \$1}")'\'''
